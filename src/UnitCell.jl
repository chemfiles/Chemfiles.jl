...
function UnitCell(a::Number, b::Number, c::Number,
    ...
    handle = lib.chrp_cell(Cdouble(a), Cdouble(b), Cdouble(c), Cdouble(alpha), Cdouble(beta), Cdouble(gamma))
    return UnitCell(handle)
end

function UnitCell(frame::Frame)
    handle = lib.chrp_cell_from_frame(frame.handle)
    return UnitCell(handle)
end
...
function cell_matrix!(cell::UnitCell, mat::Array{Cdouble, 2})
    check(
        lib.chrp_cell_matrix(cell.handle, pointer(mat))
    )
    return mat
end
...
function cell_type(cell::UnitCell)
    res = CellType[0]
    check(
        lib.chrp_cell_type(cell.handle, pointer(res))
    )
    return res[1]
end
...
function periodicity(cell::UnitCell)
    ...
    check(
        lib.chrp_cell_periodicity(cell.handle, pointer(res), pointer(res)+1, pointer(res)+2)
    )
    return res
end
